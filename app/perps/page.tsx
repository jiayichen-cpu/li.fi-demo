...
export default async function PerpsPage() {
  ...
  const entries: LeaderboardEntry[] = snapshot
    ? (() => {
        try {
          const parsed = JSON.parse(snapshot.entries);
          // Validate that parsed data is an array
          if (!Array.isArray(parsed)) {
            console.error('Invalid snapshot entries format: expected array');
            return [];
          }
          return parsed;
        } catch (error) {
          console.error('Failed to parse snapshot entries:', error);
          return [];
        }
      })()
    : [];
